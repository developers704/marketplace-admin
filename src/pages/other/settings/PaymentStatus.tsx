...
const PaymentStatus = () => {
    ...
    const deleteSelectedServices = async (serviceIds: (string | number)[]) => {
        try {
            const response = await fetch(
                `${BASE_API}/api/paymentStatus/bulk-delete`,
                {
                    method: 'DELETE',
                    headers: {
            ...
            if (!response.ok) {
                throw new Error('Failed to delete statuss')
            }

            getAllServices()
            Swal.fire({
                title: 'Deleted!',
                text: `${serviceIds.length} status(s) deleted successfully.`,
                icon: 'success',
                timer: 1500,
            })
            setSelectedRows([])
        } catch (error: any) {
            Swal.fire('Oops!', 'statuss deletion failed.', 'error')
        }
    }
    ...
    const handleBulkDelete = () => {
        Swal.fire({
            title: 'Are you sure?',
            text: `All the ${selectedRows.length} selected statuss will be deleted!`,
            icon: 'warning',
    ...
    const getAllServices = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${BASE_API}/api/paymentStatus`, {
                method: 'GET',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const handleAddService = async (serviceData: any) => {
        try {
            ...
            if (!response.ok) {
                const errorMessage = await response.json()
                throw new Error(errorMessage.message || 'Failed to add new status')
            }
    ...
    const handleUpdateService = async (serviceData: any) => {
        try {
            setApiLoading(true)
            const response = await fetch(
                `${BASE_API}/api/paymentStatus/${editingBrand?._id}`,
                {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    return (
        <>
            <PageBreadcrumb title="Payment Status" subName="Settings" allowNavigateBack={true} />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            ...
                            <p className="text-muted mb-0">
                                Add and Manage your all Payment Status here.
                            </p>
                        </div>
                        <div className="mt-3 mt-lg-0 d-flex flex-column flex-sm-row align-items-start align-items-lg-center gap-2">
                            <Button
                                ...
                                <i className="bi bi-plus"></i> Add New Status
                            </Button>
                            {showDeleteButton && (
                                <Button
                                    variant="danger"
            ...
            <Modal
                ...
                <Modal.Header closeButton>
                    <h4 className="modal-title">
                        {editingBrand ? 'Update status' : 'Add New Status'}
                    </h4>
                </Modal.Header>
                <Form
