...
const TopSection = () => {
    ...
    const handleDeleteConfirmation = (storeId: string) => {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This Top Section will be deleted!',
            icon: 'warning',
    ...
    const getStores = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${BASE_API}/api/topPageSection`, {
                method: 'GET',
                headers: {
    ...
    const handleUpdateStore = async (storeData: any) => {
        ...
        try {
            ...
            const response = await fetch(
                `${BASE_API}/api/topPageSection/${editingStore?._id}`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            Swal.fire({
                title: 'Updated!',
                text: 'Top Section updated successfully!',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const deleteStore = async (storeIds: string[]) => {
        try {
            const response = await fetch(
                `${BASE_API}/api/topPageSection/bulk-delete`,
                {
                    method: 'DELETE',
                    headers: {
            ...
            Swal.fire({
                ...
                text: storeIds.length > 1
                    ? `${storeIds.length} Top Section deleted successfully`
                    : 'Top Section deleted successfully',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const handleAddStore = async (storeData: any) => {
        ...
        try {
            ...
            if (data_res) {
                Swal.fire({
                    title: 'ADDED!',
                    text: 'Top Section added successfully!',
                    icon: 'success',
                    confirmButtonColor: "#9c5100",
                })
                getStores()
    ...
    return (
        <>
            <PageBreadcrumb title="Top Section" subName="Settings" />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            <h4 className="header-title">Top Section</h4>
                            <p className="text-muted mb-0">
                                Add and Manage all your Top Section here.
                            </p>
                        </div>
                        <div className="mt-3 mt-lg-0">
                            <Button
                                ...
                                <i className="bi bi-plus"></i> Add New Top Section
                            </Button>
                            {showDeleteButton && (
                                <Button
                                    variant="danger"
            ...
            <Modal show={isOpen} onHide={handletoggleModal} >
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editingStore ? 'Edit Top Section' : 'Add New Top Section'}
                    </Modal.Title>
                </Modal.Header>
                <Form onSubmit={handleSubmit(editingStore ? handleUpdateStore : handleAddStore)}>
                    <Modal.Body>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>Top Section Image</Form.Label>
                            <div className="mb-2">
