...
interface StoreRecord {
    _id: string
    heading: string  // Changed from text
...
const AboutUsCards = () => {
    ...
    const handleDeleteConfirmation = (storeId: string) => {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This About Page Cards will be deleted!',
            icon: 'warning',
    ...
    const filteredRecords = storeData
        ?.filter(record =>
            record?.heading?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            record?.description?.toLowerCase().includes(searchTerm.toLowerCase())
        )
        ?.sort((a, b) =>
            sortedAsc ? a?.heading?.localeCompare(b?.heading) : b?.heading?.localeCompare(a?.heading)
        )
    ...
    const toggleEditModal = (store: StoreRecord) => {
        ...
        setValue('heading', store.heading)
    ...
    const getStores = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${BASE_API}/api/aboutUsCards`, {
                method: 'GET',
                headers: {
    ...
    const handleUpdateStore = async (storeData: any) => {
        ...
        try {
            ...
            formData.append('heading', storeData.heading)
            ...
            const response = await fetch(
                `${BASE_API}/api/aboutUsCards/${editingStore?._id}`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            Swal.fire({
                title: 'Updated!',
                text: 'About Page Cards updated successfully!',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
            ...
            Swal.fire({
                title: 'Error',
                text: error.message || 'Failed to update About Page Cards',
                icon: 'error',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const deleteStore = async (storeIds: string[]) => {
        try {
            const response = await fetch(
                `${BASE_API}/api/aboutUsCards/bulk-delete`,
                {
                    method: 'DELETE',
                    headers: {
            ...
            Swal.fire({
                ...
                text: storeIds.length > 1
                    ? `${storeIds.length} About Page Cards deleted successfully`
                    : 'About Page Cards deleted successfully',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const handleAddStore = async (storeData: any) => {
        ...
        try {
            ...
            if (data_res) {
                Swal.fire({
                    title: 'ADDED!',
                    text: 'About Page Cards added successfully!',
                    icon: 'success',
                    confirmButtonColor: "#9c5100",
                })
                getStores()
    ...
    return (
        <>
            <PageBreadcrumb title="About Page Cards" subName="Settings" />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            <h4 className="header-title">About Page Cards (About Us Page)</h4>
                            <p className="text-muted mb-0">
                                Add and Manage all your About Page Cards here.
                            </p>
                        </div>
                        <div className="mt-3 mt-lg-0">
                            <Button
                                ...
                                <i className="bi bi-plus"></i> Add New About Page Cards
                            </Button>
                            {showDeleteButton && (
                                <Button
                                    variant="danger"
            ...
            <Modal show={isOpen} onHide={handletoggleModal} >
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editingStore ? 'Edit About Page Cards' : 'Add New About Page Cards'}
                    </Modal.Title>
                </Modal.Header>
                <Form onSubmit={handleSubmit(editingStore ? handleUpdateStore : handleAddStore)}>
                    <Modal.Body>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>About Page Cards Image <label style={{ color: 'red' }}>*</label></Form.Label>
                            <div className="mb-2">
