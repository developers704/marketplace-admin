...
const Section4 = () => {
    ...
    const handleDeleteConfirmation = (storeId: string) => {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This Section 4 will be deleted!',
            icon: 'warning',
    ...
    const getStores = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${BASE_API}/api/service-section-one`, {
                method: 'GET',
                headers: {
    ...
    const handleUpdateStore = async (storeData: any) => {
        ...
        try {
            ...
            const response = await fetch(
                `${BASE_API}/api/service-section-one/${editingStore?._id}`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            Swal.fire({
                title: 'Updated!',
                text: 'Section 4 updated successfully!',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
            ...
            Swal.fire({
                title: 'Error',
                text: error.message || 'Failed to update Section 4',
                icon: 'error',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const deleteStore = async (storeIds: string[]) => {
        try {
            const response = await fetch(
                `${BASE_API}/api/service-section-one/bulk`,
                {
                    method: 'DELETE',
                    headers: {
            ...
            Swal.fire({
                ...
                text: storeIds.length > 1
                    ? `${storeIds.length} Section 4 deleted successfully`
                    : 'Section 4 deleted successfully',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const handleAddStore = async (storeData: any) => {
        ...
        try {
            ...
            if (data_res) {
                Swal.fire({
                    title: 'ADDED!',
                    text: 'Section 4 added successfully!',
                    icon: 'success',
                    confirmButtonColor: "#9c5100",
                })
                getStores()
    ...
    return (
        <>
            <PageBreadcrumb title="Section 4" subName="Settings" />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            <h4 className="header-title">Section 4 (Landing Page)</h4>
                            <p className="text-muted mb-0">
                                Add and Manage all your Section 4 here.
                            </p>
                        </div>
                        <div className="mt-3 mt-lg-0">
                            <Button
                                ...
                                <i className="bi bi-plus"></i> Add New Section 4
                            </Button>
                            {showDeleteButton && (
                                <Button
                                    variant="danger"
            ...
            <Modal show={isOpen} onHide={handletoggleModal} >
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editingStore ? 'Edit Section 4' : 'Add New Section 4'}
                    </Modal.Title>
                </Modal.Header>
                <Form onSubmit={handleSubmit(editingStore ? handleUpdateStore : handleAddStore)}>
                    <Modal.Body>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>Section 4 Image <label style={{ color: 'red' }}>*</label></Form.Label>
                            <div className="mb-2">
