...
const Section8 = () => {
    ...
    const handleDeleteConfirmation = (storeId: string) => {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This Section 8 will be deleted!',
            icon: 'warning',
    ...
    const getStores = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${BASE_API}/api/petTraining`, {
                method: 'GET',
                headers: {
    ...
    const handleUpdateStore = async (storeData: any) => {
        ...
        try {
            ...
            const response = await fetch(
                `${BASE_API}/api/petTraining/${editingStore?._id}`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            Swal.fire({
                title: 'Updated!',
                text: 'Section 8 updated successfully!',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
            ...
            Swal.fire({
                title: 'Error',
                text: error.message || 'Failed to update Section 8',
                icon: 'error',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
        ...
        try {
            ...
            Swal.fire({
                ...
                text: storeIds.length > 1
                    ? `${storeIds.length} Section 8 deleted successfully`
                    : 'Section 8 deleted successfully',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const handleAddStore = async (storeData: any) => {
        ...
        try {
            ...
            if (data_res) {
                Swal.fire({
                    title: 'ADDED!',
                    text: 'Section 8 added successfully!',
                    icon: 'success',
                    confirmButtonColor: "#9c5100",
                })
                getStores()
    ...
    return (
        <>
            <PageBreadcrumb title="Section 8" subName="Settings" />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            <h4 className="header-title">Section 8 (Landing Page)</h4>
                            <p className="text-muted mb-0">
                                Add and Manage all your Section 8 here.
                            </p>
                        </div>
                        <div className="mt-3 mt-lg-0">
                            <Button
                                ...
                                <i className="bi bi-plus"></i> Add New Section 8
                            </Button>
                            {showDeleteButton && (
                                <Button
                                    variant="danger"
            ...
            <Modal show={isOpen} onHide={handletoggleModal} >
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editingStore ? 'Edit Section 8' : 'Add New Section 8'}
                    </Modal.Title>
                </Modal.Header>
                <Form onSubmit={handleSubmit(editingStore ? handleUpdateStore : handleAddStore)}>
                    <Modal.Body>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>Section 8 Image <label style={{ color: 'red' }}>*</label></Form.Label>
                            <div className="mb-2">
