...
const Section22 = () => {
    ...
    const handleDeleteConfirmation = (storeId: string) => {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This last Section will be deleted!',
            icon: 'warning',
    ...
    const getStores = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${BASE_API}/api/keepingYouInTheKnow`, {
                method: 'GET',
                headers: {
            ...
            toastService.error({
                status: error.status,
                message: error.message || 'Failed to fetch last Section'
            })
        } finally {
            setLoading(false)
        }
    ...
    const handleUpdateStore = async (storeData: any) => {
        ...
        try {
            ...
            const response = await fetch(
                `${BASE_API}/api/keepingYouInTheKnow/${editingStore?._id}`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (!response.ok) {
                ...
                throw new Error(errorMessage.message || 'Failed to update last Section')
            }
            ...
            Swal.fire({
                title: 'Updated!',
                text: 'last Section updated successfully!',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
            ...
            Swal.fire({
                title: 'Error',
                text: error.message || 'Failed to update last Section',
                icon: 'error',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const deleteStore = async (storeIds: string[]) => {
        try {
            const response = await fetch(
                `${BASE_API}/api/keepingYouInTheKnow/bulk-delete`,
                {
                    method: 'DELETE',
                    headers: {
            ...
            Swal.fire({
                ...
                text: storeIds.length > 1
                    ? `${storeIds.length} last Section deleted successfully`
                    : 'last Section deleted successfully',
                icon: 'success',
                timer: 1500,
                confirmButtonColor: "#9c5100",
            })
    ...
    const handleAddStore = async (storeData: any) => {
        ...
        try {
            ...
            if (data_res) {
                Swal.fire({
                    title: 'ADDED!',
                    text: 'last Section added successfully!',
                    icon: 'success',
                    confirmButtonColor: "#9c5100",
                })
                getStores()
    ...
    return (
        <>
            <PageBreadcrumb title="Section 22" subName="Settings" />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            <h4 className="header-title">Section 22 (Landing Page)</h4>
                            <p className="text-muted mb-0">
                                Add and Manage all your Section 22 here.
                            </p>
                        </div>
                        <div className="mt-3 mt-lg-0">
                            <Button
                                ...
                                <i className="bi bi-plus"></i> Add New Section 22
                            </Button>
                            {showDeleteButton && (
                                <Button
                                    variant="danger"
            ...
            <Modal show={isOpen} onHide={handletoggleModal} >
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editingStore ? 'Edit Section 22' : 'Add New Section 22'}
                    </Modal.Title>
                </Modal.Header>
                <Form onSubmit={handleSubmit(editingStore ? handleUpdateStore : handleAddStore)}>
                    <Modal.Body>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>Section 22 Image</Form.Label>
                            <div className="mb-2">
