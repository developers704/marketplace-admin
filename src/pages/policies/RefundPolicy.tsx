...
const RefundPolicy = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (existingTermsId) {
                const response = await fetch(`${BASE_API}/api/policies/refund/${existingTermsId}`, {
                    method: 'PUT',
                    headers: {
                ...
                const response = await fetch(`${BASE_API}/api/policies/refund`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${token}`,
                ...
                setExistingTermsId(data.refundPolicy._id)
                Swal.fire({
                    title: 'Success!',
                    text: data.message,
                    icon: 'success',
    ...
    return (
        <>
            <PageBreadcrumb title="Refund & Exchange Policy" subName="Policies" />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            <h4 className="header-title">Refund & Exchange Policy</h4>
                            <p className="text-muted mb-0">
                                Add and Manage your Refund & Exchange Policy here.
                            </p>
                        </div>
                    </div>
                </Card.Header>
                        ...
                        <div className="text-end mt-3">
                            ...
                            >
                                {apiloading ? (
                                    ...
                                    <span>
                                        {existingTermsId ? 'Update Refund Policy' : 'Save Refund Policy'}
                                    </span>
                                )}
                            </Button>
                        </div>
