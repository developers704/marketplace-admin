...
const DayCare = () => {
    ...
    const [isPackageModalOpen, togglePackageModal] = useToggle()
    ...
    const toggleEditModal = (service: TableRecord) => {
        ...
        setValue('numberOfDays', service.numberOfDays)

        const selectedPackage = packageOptions.find(option => option.value === service.package._id)
        if (selectedPackage) {
    ...
    const handleAllPetsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.checked) {
            // Select all pets in the dropdown
    ...
    const getAllDayCareServices = async () => {
        try {
            setLoading(true)
            const response = await fetch(`${BASE_API}/api/daycare`, {
                method: 'GET',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorMessage.message || 'Failed to get daycare services')
            }
            const data: TableRecord[] = await response.json()
            setBrandsData(data)
        } catch (error: any) {
            console.error('Error getting daycare services:', error)
        } finally {
            setLoading(false)
        }
    }

    const handleAddDayCare = async (daycareData: any) => {
        const formData = new FormData()
        formData.append('name', daycareData.name)
        formData.append('price', daycareData.price)
        formData.append('description', daycareData.description)
        formData.append('numberOfDays', daycareData.numberOfDays)

        if (daycareData.package) {
            formData.append('package', daycareData.package.value)
        }
        if (daycareData.petNames) {
            daycareData.petNames.forEach((pet: any) => {
                formData.append('petNames', pet.value)
            })
        }
        if (daycareData.locations) {
            daycareData.locations.forEach((location: any) => {
                formData.append('locations', location.value)
            })
        }
        if (daycareData.service) {
            formData.append('service', daycareData.service.value)
        }
        ...
        try {
            ...
            Swal.fire({
                title: 'Success!',
                text: 'Daycare service added successfully!',
                icon: 'success',
                timer: 1500,
            })
            handleToggleModal()
            getAllDayCareServices()
        } catch (error: any) {
            Swal.fire({
                title: 'Oops!',
                text: error.message,
    ...
    const handleUpdateDayCare = async (daycareData: any) => {
        const formData = new FormData()
        formData.append('name', daycareData.name)
        formData.append('price', daycareData.price)
        formData.append('description', daycareData.description)
        ...
        try {
            ...
            const response = await fetch(
                `${BASE_API}/api/daycare/${editingBrand?._id}`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            Swal.fire({
                title: 'Updated!',
                text: 'Daycare service updated successfully!',
                icon: 'success',
                timer: 1500,
            })
            handleToggleModal()
    ...
    const deleteServices = async (ids: number[]) => {
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to delete daycare services')
            }
            ...
            Swal.fire({
                title: 'Deleted!',
                text: `Daycare service(s) deleted successfully.`,
                icon: 'success',
                timer: 1500,
            })
        } catch (error: any) {
    ...
    const getMainServices = async () => {
        try {
            ...
            // Find and set the DayCare Service option as default
            const daycareService = options.find((option: any) =>
                option.label.toLowerCase().includes('day care')
            )
            if (daycareService) {
                setValue('service', daycareService)
            }
        } catch (error) {
    ...
    const getPackages = async () => {
        try {
            ...
            const data = await response.json()
            const options = data.map((pkg: any) => ({
                value: pkg._id,
                label: pkg.name
            }))
    ...
    const handleAddPackage = async (formData: any) => {
        try {
            ...
            if (!response.ok) {
                const errorMessage = await response.json()
                throw new Error(errorMessage.message || 'Failed to add package')
            }
    ...
    useEffect(() => {
        if (!isOpen) {
            ...
            const daycareService = mainServiceOptions.find(option =>
                option.label.toLowerCase().includes('day care')
            )
            if (daycareService) {
                setValue('service', daycareService)
    ...
    return (
        <>
            <PageBreadcrumb title="Day Care Service" subName="Services" />
            <Card>
                <Card.Header>
                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
                        <div>
                            <h4 className="header-title">Day Care Service Management</h4>
                            <p className="text-muted mb-0">
                                Add and Manage your all day care service here.
                            </p>
                        </div>
                ...
                <Modal
                    ...
                    <Form
                        onSubmit={handleSubmit(
                            editingBrand ? handleUpdateDayCare : handleAddDayCare
                        )}>
                        <Modal.Body>
                            <Row>
                                <Col md={6}>
                                    <Form.Group className="mb-3">
                                        <Form.Label className='d-flex align-items-center'>Main Service <span className="text-danger ms-1">*</span></Form.Label>
                                        <Select
                                            value={watch('service')}
                                ...
                                <Col md={6}>
                                    <Form.Group className="mb-3">
                                        <Form.Label className='d-flex align-items-center'>Title <span className="text-danger ms-1">*</span></Form.Label>
                                        <FormInput
                                            type="text"
                                            name="name"
                                            containerClass="mb-3"
                            ...
                            <Row>
                                ...
                                <Col md={6}>
                                    <Form.Group className="mb-3">
                                        <Form.Label className='d-flex align-items-center'>Number Of Days <span className="text-danger ms-1">*</span></Form.Label>
                                        <FormInput
                                            type="number"
                                            name="numberOfDays"
                                            containerClass="mb-3"
                                ...
                                <Col md={12}>
                                    <Form.Group className="mb-3">
                                        <div className="d-flex justify-content-between align-items-center">
                                            <Form.Label className='d-flex align-items-center'>Package Name <span className="text-danger ms-1">*</span></Form.Label>
                ...
                <Modal
                    ...
                    <Modal.Body>
                        <Form onSubmit={handleSubmit(handleAddPackage)}>
                            ...
                            <Modal.Footer>
                                <Button variant="light" onClick={togglePackageModal}>
                                    Close
                                </Button>
                                <Button variant="soft-success" type="submit">
                                    {apiLoading ? 'Adding...' : 'Save Package'}
                                </Button>
                            </Modal.Footer>
                        </Form>
                    </Modal.Body>
